...
const char *productLabels[NUM_PRODUCTS] = {"Hershey's Milk Chocolate Bar 1.55oz", "Lays Classic Chips 13oz", "Orbit Chewing Gum 15 sticks", "Pepsi Soda 20oz Bottle"};
...
void displayProducts(const Product products[NUM_PRODUCTS])
{
    ...
    for(i = 0; i < NUM_PRODUCTS; i++)
    {
        printf("%-4d | %-35s | %-18d | %-10.2f\n", i, productLabels[products[i].productType], products[i].quantityInStock, products[i].price);
    }
}
...
ProductType getProduct(void)
{
    ...
    while(!hasSelection)
    {
        ...
        else
        {
            ...
            {
                selectedProduct = (int)parsedValue;

                if(selectedProduct >= 0 && selectedProduct < NUM_PRODUCTS)
                {
...
void generateBill(const ShoppingCart *cart)
{
    ...
    for(i = 0; i < cart->totalItems; i++)
    {
        ...
        printf(" %d x %-34s | %2d x %4.2f  | %.2f\n", current->quantity, productLabels[current->item.productType], current->quantity, current->item.price, lineTotal);
    }
...
bool pay(const ShoppingCart *cart)
{
    ...
    {
        printf("Input 0 for Cash, 1 for Credit Card, or 2 for Contactless: ");
        ...
        if(endPtr != input && (*endPtr == '\n' || *endPtr == '\0'))
        {
            paymentOption = (int)parsedValue;

            if(paymentOption >= CASH && paymentOption <= CONTACTLESS)
            {
                methodSelected = true;
            }
            else
    ...
    else if(paymentOption == CREDIT)
    {
        ...
        while(!cardSelected)
        {
            ...
            if(endPtr != input && (*endPtr == '\n' || *endPtr == '\0'))
            {
                cardOption = (int)parsedValue;

                if(cardOption >= MASTERCARD && cardOption <= AMERICAN_EXPRESS)
                {
                    cardSelected = true;
                }
                else
    ...
    else
    {
        ...
        while(!mobileSelected)
        {
            ...
            if(endPtr != input && (*endPtr == '\n' || *endPtr == '\0'))
            {
                mobileOption = (int)parsedValue;

                if(mobileOption >= GOOGLE_WALLET && mobileOption <= APPLE_PAY)
                {
                    mobileSelected = true;
                }
                else
